#!/usr/bin/env bash
# Run the ABC route (B-RASP -> 2LTL -> PVWAA -> circuit -> abc pdr) over
# .brasp examples, one CSV row per instance: verdict plus the --timing phase
# breakdown. Same columns as run_abc.sh, so the two CSVs line up column-for-
# column — that script starts from already-compiled .ltl, this one from the
# B-RASP source, so `compile_s` here also covers the B-RASP -> 2LTL front end.
#
#   scripts/run_brasp.sh                            # every examples/brasp/*.brasp
#   scripts/run_brasp.sh examples/brasp/y_depth__no_two_a__k-64000.brasp  # just these
#   TIMEOUT=60 scripts/run_brasp.sh                 # longer budget
#
...
TIMEOUT=${TIMEOUT:-120}
OUT=${OUT:-results/brasp_examples.csv}
REPS=${REPS:-1}

# GNU timeout is `gtimeout` where coreutils is installed under its g- prefix.
if command -v timeout >/dev/null 2>&1; then TIMEOUT_BIN=timeout
elif command -v gtimeout >/dev/null 2>&1; then TIMEOUT_BIN=gtimeout
else echo "run_brasp.sh: need GNU timeout (brew install coreutils)" >&2; exit 1
fi

[ -f "$JAR" ] || { echo "run_brasp.sh: no jar at $JAR — run 'sbt assembly' first" >&2; exit 1; }
[ -x "$ABC_BIN" ] || { echo "run_brasp.sh: no abc binary at $ABC_BIN — set ABC_BIN" >&2; exit 1; }

files=("$@")
if [ ${#files[@]} -eq 0 ]; then files=(examples/brasp/*.brasp); fi
...
TMP=$(mktemp "${OUT}.XXXXXX") || { echo "run_brasp.sh: could not create temp file next to $OUT" >&2; exit 1; }
trap 'rm -f "$TMP"' EXIT
...
for f in "${files[@]}"; do
  ...
  [ -f "$f" ] || { echo "run_brasp.sh: skipping missing $f" >&2; skipped=$((skipped+1)); continue; }
  stem=$(basename "$f" .brasp)
